// =============================================================================
// k16A2rep.cpp — automorphism-REPRESENTATIVE method for classifying K16 Perfect
// 1-Factorizations (P1Fs) by a given automorphism order. N=16 sibling of
// k18a2rep.cpp (the engine is N-generic; this file differs only in N and names).
//
// Why this exists: the cyclic K16A2 search cannot finish small automorphism
// orders (L=4, L=2), AND it structurally undercounts even orders that have
// FIXED-POINT-FREE cycle types (e.g. order-4 4^4 = 16), because its generator
// forces vertex 0 to be a fixed point of the automorphism. This module is
// unseeded and complete: for ONE representative permutation alpha per cycle
// TYPE of the target order, it finds ALL alpha-invariant P1Fs by an exact cover
// of the 120 edges with alpha-invariant factor-orbits, then deduplicates across
// types with a labeling-independent canonical key.
//
// Ground-truth K16 counts (Wanless catalogue, by-|Aut| histogram of |Aut|>2):
// {3:19, 5:5, 7:4, 14:1, 15:1} = 30. By order-having: order-3 -> 20, order-5 ->
// 6, order-7 -> 5. Standalone validation: order-5 -> 6, order-7 -> 5, order-15
// -> 1. Isomorph rejection is by ORDERLY GENERATION: at every cover level the
// candidate factor-orbits covering the lowest uncovered edge are deduplicated by
// the current centralizer stabilizer G, and each chosen orbit O recurses with its
// sub-stabilizer Stab_G(O). Sound for class counting because any g in G =
// Stab(partial cover) is a class-preserving bijection between sibling subtrees;
// combined with the shrinking stabilizer chain this collapses the dense
// high-centralizer types (e.g. order-4 4^4) that the previous naive lex-leader
// prune could not terminate.
//
...
//   * The ONLY entry point is the private member K16A2::runRepresentativeMethod,
//     called internally from K16A2::runExhaustiveSearch. Everything else lives in
//     the anonymous namespace below, so nothing new has external linkage.
//   * Worker count comes from K16A2::kThreads.
//   * Every print is gated by K16A2::m_bPrint (forwarded as g_bprint).
// =============================================================================
...
constexpr int N      = 16;             // players / graph vertices (K16)
constexpr int NM     = N - 1;          // 15 = factors in a 1-factorization
constexpr int NHALF  = N / 2;          // 8  = edges in one perfect matching (factor)
constexpr int NEDGES = N * (N - 1) / 2;// 120 = edges of K16
...
int     eidT[N][N];          // eidT[u][v] = canonical id (0..119) of edge {u,v}; symmetric
...
bool g_bprint = false;                // mirror of K16A2::m_bPrint (gate for ALL prints)
...
// =============================================================================
// Private K16A2 entry point (declared in k16A2.h, called only from
// runExhaustiveSearch). Enumerates all K16 P1Fs with an automorphism of `order`,
...
// =============================================================================
void K16A2::runRepresentativeMethod(int order, int target) {
    initEdges();
    const long long symCap = K16_REP_SYMCAP;
    ...
    if (m_bPrint) {
        printf("[K16-REP] order=%d : %zu cycle types, kThreads=%d, symCap=%lld\n", order, types.size(), nThreads, symCap);
        if (target > 0) printf("[K16-REP] HARVEST MODE: stop after %d distinct classes (COMPLETE only if %d == true count, else a LOWER BOUND)\n", target, target);
        fflush(stdout);
    }
    ...
    for (auto& type : types) {
        ...
        if (m_bPrint) {  // [diag] dedup finished; show task reduction and stabilizer size before the cover
            ...
            printf("[K16-REP]  type %-14s rawTasks %zu uniq %zu reps %zu stab0 %zu (dedup %.1fs) -> cover...\n",
        ...
        if (m_bPrint) {
            ...
            printf("[K16-REP]  type %-14s |C|=%-8zu rawTasks %-7zu reps %-7zu new +%zu (cum %zu) %.1fs\n",
