...
void K1111P1F::parallel_for(int start, int end, std::function<void(int, int)> task, int grain_size) {
    ...
    parallel_for(0, np, [&](int i, int tid) {
        ...
        for (int p_idx = 0; p_idx < 6; p_idx++) {
            ...
            for (int k = 0; k < info.count; k++) {
                ...
                for (int m = 0; m < 4; m++) {
                    uint8_t tr_m[24];
                    apply_perm_22(r_factors4[m]->adj, perm, tr_m);
                ...
                if (identical) {
                    ...
                    bool trivial = true;
                    for (int n = 0; n < 22; n++) if (perm.p[n] != n) { trivial = false; break; }
...
bool K1111P1F::is_perfect_scalar(const uint8_t * adj1, const uint8_t * adj2) {
    // Hamiltonian cycle check for K(11,11) -> Cycle of length 22
...
void K1111P1F::get_transformations(const Factor & fi, const Factor & fj, TransInfo & info) {
    info.count = 0;
    CycleUnion source_cu = find_cycles(fi.adj, fj.adj);
    if (source_cu.count != 1 || source_cu.lens[0] != 22) return;
    if (target_cu.count != 1 || target_cu.lens[0] != 22) return;
    ...
    for (int dir = 0; dir < 2; dir++) {
        for (int start_offset = 0; start_offset < 22; start_offset++) {
            ...
            for (int i = 0; i < 22; i++) {
                int src_pos = (dir == 0) ? (start_offset + i) % 22 : (start_offset - i + 22) % 22;
...
#if K1111_CHECK_BIPARTITE_CONSISTENCY
    for (int k = 0; k < info.count; ++k) {
...
void K1111P1F::apply_perm_22(const uint8_t * src_adj, const Permutation& perm, uint8_t * dst_adj) {
    ...
    // Safe load for tail (vertices 16..21). 22 bytes total.
    ...
    // Tail (16..21). Do not use _mm_storel_epi64 to avoid overflow!
    uint64_t tail;
    _mm_storel_epi64((__m128i*)&tail, _mm256_extracti128_si256(v_final, 1));
...
KBipartBase::FastSortedFactor K1111P1F::get_fast_sorted(const uint8_t * adj) {
    ...
    uint64_t tail_safe_v = 0; memcpy(&tail_safe_v, adj + 16, 6);
    v_adj = _mm256_inserti128_si256(v_adj, _mm_loadl_epi64((const __m128i*)&tail_safe_v), 1);
    uint32_t mask = _mm256_movemask_epi8(_mm256_cmpgt_epi8(v_adj, v_indices)) & 0x003FFFFF;

    FastSortedFactor sf; memset(&sf, 0, sizeof(sf));
    uint8_t* out = sf.pairs;

    auto process_chunk = [&](uint32_t m, const uint8_t* adj_ptr, int offset, bool safe_tail) {
        __m128i v_adj_chunk;
        if (safe_tail) {
            uint64_t tmp = 0; memcpy(&tmp, adj_ptr, 6); 
            v_adj_chunk = _mm_loadl_epi64((const __m128i*)&tmp);
        } else {
            v_adj_chunk = _mm_loadl_epi64((const __m128i*)adj_ptr);
        }
    ...
    process_chunk(mask & 0xFF, adj, 0, false);
    process_chunk((mask >> 8) & 0xFF, adj + 8, 8, false);
    process_chunk((mask >> 16) & 0xFF, adj + 16, 16, true);

#if 0 // Debug verification
    FastSortedFactor ref; int count = 0;
    for (int i = 0; i < 22; ++i) if (i < adj[i]) {
        ref.pairs[count * 2] = (uint8_t)i; ref.pairs[count * 2 + 1] = adj[i]; count++;
    }
    if (memcmp(sf.pairs, ref.pairs, 22) != 0) {
        printf("GFS Error!\n");
    }
#endif
    return sf;
}
bool K1111P1F::is_canonical(int r4_fid, int r5_fid, const K1111P1F::TransInfo * r4_dependent_trans) {
    const uint8_t* r_adj[5] = {
        fixedRows[0].adj, fixedRows[1].adj, fixedRows[2].adj,
        global_pool[r4_fid].adj, global_pool[r5_fid].adj
    };

    K1111P1F::FastRowTriplet target_triplet;
    ...
    auto check_trans = [&](const K1111P1F::TransInfo& info, int i, int j) {
        for (int k = 0; k < info.count; ++k) {
            const auto& perm = info.perms[k];
            uint8_t tr_fi[22], tr_fj[22];
            apply_perm_22(r_adj[i], perm, tr_fi);
            apply_perm_22(r_adj[j], perm, tr_fj);
...
bool K1111P1F::is_canonical_stab(int r5_fid, const Permutation* stab, int stab_count) {
    ...
    for (int k = 0; k < stab_count; k++) {
        ...
        uint8_t tr_r5[22];
        apply_perm_22(f5.adj, perm, tr_r5);
...
void K1111P1F::diagnostic_printout(double current_compr) {
