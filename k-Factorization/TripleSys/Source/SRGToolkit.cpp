...
int SRGToolkit::canonizeGraph(ctchar* pGraph, tchar* pGraphOut, int firstVert) {
	...
	m_pNumOrbits[0] = 1;
	m_pLenOrbits[0] = m_v;
	auto pLenOrbits = m_pLenOrbits;
	auto pOrbits = m_pOrbits;
	int len, lastUnfixedVertexIndex = 0;
	int idxRight = 1;  // Continue, until at least one orbit length > 1
	int i = firstVert;
	if (i) {// The smallest index of the vertex that will be considered first
		auto pLenOrbitsNext = m_pLenOrbits;
		pLenOrbitsNext += i * (m_v - 1) - i * (i - 1) / 2;
		pLenOrbits = pLenOrbitsNext - m_v + i;
		m_pOrbits[m_pOrbits[i] = i + 1] = i;  // swapping of i-th and (i+1)-th vertices
		flag = canonizeMatrixRow(pGraph, pGraphOut, i,
			&pLenOrbits/*pLenOrbitsNext - m_v + i*/, idxRight, flag, lastUnfixedVertexIndex);
	}
	...
	return flag > 0 || !firstVert ? lastUnfixedVertexIndex : 0;
}

int SRGToolkit::canonizeMatrixRow(ctchar* pGraph, tchar* pVertOut, int vertIdx,
	tchar ** ppLenOrbits, int& idxRight, bool flag, int &lastUnfixedVertexIndex) {

	auto idxLast = vertIdx;
	const auto vertIdxNext = vertIdx + 1;
	const auto len = m_v - vertIdxNext;
...
/*	if (vertIdx < 10) {
		FOPEN_F(f, "bbb.txt", "a");
		fprintf(f, "%2d: pGraph = %p pLenOrbits = %p (%p)\n", i, (firstVert ? pGraphOut : pGraph) + i * m_v, pLenOrbits, pLenOrbitsNext);
		FCLOSE_F(f);
	} */
