...
void K16P1F::init(int fixed3RowsIndex, int kThreads, const unsigned char* first3Rows, ResultCallback callback, void* cbClassPtr, bool bPrint) {
...
bool K16P1F::addRow(int iRow, const unsigned char* source) {
    if (iRow < K16_FIXED || iRow >= K16_MATCH) return false;
    int slot = iRow - K16_FIXED;
...
void K16P1F::solve() {
    ...
    parallel_for(0, (int)r4_groups.size(), [&](int group_idx, int tid) {
        ...
        int s4_sw[K16_SEARCH], s4_ew[K16_SEARCH];
        int s4_l_cur = 0;
        for (int r = 2; r < K16_SEARCH; r++) {
...
void K16P1F::apply_perm_16(const uint8_t* src_adj, const Permutation24& perm, uint8_t* dst_adj) {
...
bool K16P1F::is_canonical_stab(int r5_fid, const Permutation24* stab, int stab_count) {
