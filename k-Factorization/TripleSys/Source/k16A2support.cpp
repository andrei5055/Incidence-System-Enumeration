...
struct FlatMatchingMap {
    ...
    void build(const std::vector<std::array<uint8_t, NP>>& pool) {
        int cap = 1;
    ...
    int find(const std::array<uint8_t, NP>& m) const {
        uint64_t h = hash_m(m);
        int idx = h & mask;
        while (table[idx].value != -1) {
            if (table[idx].key == m) {
...
private:
    static FORCE_INLINE uint64_t hash_m(const std::array<uint8_t, NP>& m) {
        uint64_t h1, h2;
        memcpy(&h1, m.data(), 8);
        ...
        if (t_depth > 0) {
            ...
            if (!nodes.empty() && (int)nodes.size() < self->kThreads) {
                ...
                for (auto& nd : nodes) {
                    uint8_t local_c[NP];
                    bool local_used[NP];
                    memcpy(local_c, nd.c, NP);
                    memcpy(local_used, nd.used, NP);
                ...
                for (int t = 0; t < self->kThreads; t++) {
                    workers.emplace_back([&]() {
                        ...
                        while (true) {
                            ...
                            memcpy(local_c, nodes[idx].c, NP);
                            memcpy(local_used, nodes[idx].used, NP);
...
void K16A2::CycleBacktrackState::generate_remaining_cycles(int start_idx, const uint8_t* rem, int rem_size, bool* rem_used, uint8_t* alpha_p, int L, int depth) {
    ...
    for (int d = 1; d <= rem_size; d++) {
        ...
        if (d == 1) {
            ...
            bool defined[NP];
            for (int i = 0; i < NP; i++) defined[i] = true;
            ...
            if (unused_count >= d - 1) {
                uint8_t cycle_nodes[NP];
                cycle_nodes[0] = v0_rem;
                
                int perm[NP];
                bool perm_used[NP] = { false };
    ...
    for (int w = 0; w < self->kThreads; w++) {
        workers.emplace_back([&]() {
            ...
            while (true) {
                ...
                uint8_t local_alpha[NP];
                bool local_rem_used[NP] = { false };
                memcpy(local_alpha, tasks[i].alpha_p, NP);
...
void K16A2::runExhaustiveSearch() {
    ...
    for (int L = NM; L >= 1; L--) {
        ...
        if (L == 1) {
            ...
            sprintf_s(cycle_str, sizeof(cycle_str), "(5)^k(1)^{NP-5k} for k in {1..3}");
        } else if (L == 3) {
            sprintf_s(cycle_str, sizeof(cycle_str), "(3)^k(1)^{NP-3k} for k in {1..6}");
        } else if (L == 2) {
            sprintf_s(cycle_str, sizeof(cycle_str), "(2)^k(1)^{NP-2k} for k in {1..NHALF}");
        } else { // L = 12, 10, 8, 6, 4
...
void K16A2::verifyL16Pairing(const std::set<std::vector<uint8_t>>& local_unique) {
    ...
    for (int i = 0; i < n; i++) {
        ...
        memset(H, 0, sizeof(H));
        for (int k = 1; k <= NM; k++) {
            const uint8_t* src = mats[i].data() + (k - 1) * NP;
...
void K16A2::processAutomorphism(const std::array<uint8_t, NP>& alpha_arr, int L, std::set<std::vector<uint8_t>>& unique_results) {
    ...
    // L < NM: a seed orbit may admit several completions; record EVERY one of them
    // (the canonizer + final dedup collapse isomorphic duplicates). Previously only the
    // first cover was kept, which silently dropped the other valid P1Fs.
    uint8_t G[NM][NP];
    memcpy(G[0], fixedRows[0].adj, NP);
    constructFullH(G, L, alpha, G);
    uint8_t Hdummy[NP][NP];
    decomposeMissingEdges(G, L, alpha, Hdummy, [&](const uint8_t H[][NP]) {
        recordIsomorphicResults(H, unique_results);
    });
}
...
bool K16A2::decomposeMissingEdges(const uint8_t G[][NP], int L, const uint8_t* alpha_p, uint8_t H[][NP],
    ...
    bool complement_adj[NP][NP];
    memset(complement_adj, false, sizeof(complement_adj));
    uint8_t comp_neighbors_higher[NP][NP];
    uint8_t comp_neighbors_higher_count[NP];
    ...
    static thread_local std::vector<std::array<uint8_t, NP>> pool;
    ...
    auto generate_matchings = [&](auto& self_fn, int u, int edge_count) -> void {
        ...
        if (u == NP) {
            std::array<uint8_t, NP> m;
            memcpy(m.data(), current_matching, NP);
            pool.push_back(m);
            return;
        }
        if (used[u]) {
    ...
    for (int i = 0; i < (int)pool.size(); i++) {
        ...
        std::array<uint8_t, NP> curr = pool[i];
    ...
    // 5. Map each complement edge to an index 0..NEDGES
    std::pair<uint8_t, uint8_t> comp_edges[NEDGES];
    int comp_edge_cnt = 0;
    int edge_to_idx[NP][NP];
    ...
    auto solve_exact_cover_orbits = [&](auto& self_fn) -> bool {
        ...
        if (current_size_sum == num_colors) {
            if (onCover) {
                // A seed orbit can have several valid completions; build this cover's H,
                // deliver it, then keep backtracking to enumerate the rest.
                uint8_t matchings[NM][NP];
                memset(matchings, 0xFF, sizeof(matchings));
                int m_count = 0;
                for (int o_idx : chosen_orbits)
                    for (int m_idx : orbits[o_idx].matching_indices)
                        memcpy(matchings[m_count++], pool[m_idx].data(), NP);
                uint8_t Hc[NP][NP];
                copyMatchingsToH(matchings, num_colors, G, L, Hc);
                onCover(Hc);
                anyCover = true;
                return false;   // continue searching for further covers
            }
            return true;
        }
...
bool K16A2::is_color_compatible(int c, int num_colors, uint8_t matchings[][NP], const uint8_t* G0) {
    ...
    if (count < NP) return true; // Not yet complete
...
void K16A2::applyPermToH(const uint8_t H[][NP], const uint8_t* p, uint8_t S[][NP]) {
    for (int k = 1; k <= NM; k++) {
        ...
        memcpy(S[nb], permuted_factor, NP);
    }
}
...
void K16A2::recordS(const uint8_t S[][NP], std::set<std::vector<uint8_t>>& unique_results) {
    ...
    std::vector<uint8_t> res_vec(results, results + NM * NP);
