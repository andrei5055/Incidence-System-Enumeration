...
void K18A2::runRepresentativeMethod(int order, int target) {
    initEdges();
    const long long symCap = K18_REP_SYMCAP;
    ...
    if (m_bPrint) {
        printf("[K18-REP] order=%d : %zu cycle types, kThreads=%d, symCap=%lld\n", order, types.size(), nThreads, symCap);
    ...
    for (auto& type : types) {
        ...
        if (m_bPrint) {  // [diag] dedup finished; show task reduction and stabilizer size before the cover
            ...
            printf("[K18-REP]  type %-14s rawTasks %zu uniq %zu reps %zu stab0 %zu (dedup %.1fs) -> cover...\n",
        ...
        if (m_bPrint) {
            ...
            printf("[K18-REP]  type %-14s |C|=%-8zu rawTasks %-7zu reps %-7zu new +%zu (cum %zu) %.1fs\n",
