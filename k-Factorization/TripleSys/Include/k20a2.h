...
#define VECTOR_CALL
#endif
#define K20_Use_rdtsc 0
#define K20_N 16
#define K20_MATCH 15
#define K20_FIXED 3
#define K20_SEARCH 12
#define K20_M_MAX 60000
#define K20_SORT_INPUT 0
#define K20_USE_ROOT_SORT        1

#define K20_DISABLE_IS_CANONICAL_R45_CHECK 0
#define K20_BENCHMARK_EXIT     0

#define K20_EDGE_PRUNE_ENABLED 1
#define K20_EDGE_PRUNE_START   9 // 9
#define K20_EDGE_PRUNE_END     12 // 12
#define K20_PRUNE_THRESHOLD_HIGH 20000
#define K20_PRUNE_THRESHOLD_LOW  500 // 500

#define K20_USE_MRV              1
#define K20_MRV_DEPTH_LIMIT      8 // Only reorder for depths < 8 
#define K20_MRV_EARLY_EXIT       1
#define K20_MRV_EARLY_EXIT_THRESHOLD 1

#if !defined(K16P1F_H) && !defined(K16A2_H) && !defined(K20P1F_H)
struct Mask256_C { uint64_t m[2] = { 0, 0 }; };
#endif

class K20A2 : public KBase<Mask256_C> {
public:
    ...
    #define K20_WORDS (((K20_M_MAX + 255) / 256 * 4) + 4)
    struct State { alignas(64) uint64_t bits[K20_WORDS]; };

    struct SearchContext {
        alignas(64) State pool[K20_SEARCH + 1];
        Mask256_C used_edges;
        uint8_t slots[K20_SEARCH];
        int r4_idx = 0;
        int r5_idx = 0;
        int root_idx = 0;
        uint16_t mrv_counts[K20_SEARCH + 1][K20_SEARCH]; // [depth][slot_idx]
        bool counts_valid[K20_SEARCH + 1];
    };
    ...
    struct alignas(64) ThreadLocalBuffers : LocalBufers<Mask256_C> {
        ...
        uint64_t r5_row_mask_in_s4[K20_SEARCH][K20_WORDS];
        int dirty_s4_words[K20_SEARCH][K20_WORDS];
        int dirty_s4_count[K20_SEARCH];
        SubSamplePlanItem sub_sampling_plans[K20_SEARCH][K20_WORDS];
        int sub_sampling_plan_sizes[K20_SEARCH];
        int s4_bit_cursor_table[K20_SEARCH];
    ...
    K20A2(const FactorParams& factParam, int fixed3RowsIndex, int kThreads, const unsigned char* first3Rows, ResultCallback callback, void* cbClassPtr = NULL, bool bPrint = true);
    ~K20A2() {}
...
private:
    ...
    Factor fixedRows[K20_FIXED];
    PackedAdj fixed_packed[K20_FIXED];
    Mask256_C fixedEdgesMask;
    int edge_id_table[K20_N][K20_N];
    ...
    FORCE_INLINE bool check_cycle_simd(__m128i v_e2o1, __m128i v_o2e2, __m128i v_id) {
