...
CC void CRowStorage::initCompatibilityMasks(ctchar* u1fCycles) {
	...
	if (true || !m_pAllData) {
		m_pRowSolutionMasksIdx = new uint[numPlayers()];
		m_pRowSolutionMasks = new tmask[numPlayers()];
		memset(m_pRowSolutionMasks, 0, numPlayers() * sizeof(m_pRowSolutionMasks[0]));
