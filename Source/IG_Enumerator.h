...
public:
	CK CIG_Enumerator(const C_InSys<T> *pBIBD, unsigned int enumFlags = t_enumDefault, bool firstPath = false, int treadIdx = -1, uint nCanonChecker = 0) :
		...
		m_pRowIntersections = lambdaSet->GetSize() > 2 || lambdaSet->GetAt(0) || lambdaSet->GetAt(1) != 1?
							  new T[lenRowIntersection(rowNumb())] : NULL;

		// Allocate memory to store current lambdaA, lambdaB for blocks
		// This set of lambda's is not known, but it cannot have more than k elements
		const auto len = this->getInSys()->GetK() + 1;
		m_pLambda[1] = (m_pLambda[0] = new T[2 * len]) + len;
	}

	CK ~CIG_Enumerator()								{ delete[] rowIntersections(); delete[] lambdaA(); }
	...
	inline T *lambdaA() const							{ return m_pLambda[0]; }
	inline T *lambdaB() const							{ return m_pLambda[1]; }
protected:
